...
module.exports = (client) => {
	...
	const clearChannel = async (channel) => await channel.messages.fetch({limit: 100}).then(messages => {
	...
	for(let role = 0; role < roles.length; role++) {
		client.channels.cache.get('846459783674593371').send((embeds[role])).then(async msg => {
